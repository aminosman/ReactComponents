...
type Option = { key: number | string, value: string }
type Options<T> = [keyof T, Option[] | undefined | null]
type ItemOptions<T> = Map<keyof T, Option[] | undefined | null>
...
const TableLoader = <T extends object>(props: Props<T>) => {
    ...
    const renderHeader = () => (
        <tr className="bg-dark text-white">
            {props.schema.map(i => <th key={`row-header-${i.label}`}>{i.label}</th>)}
            {props.onRemove && <th />}

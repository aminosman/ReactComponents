...
const TableLoader = <T extends object>(props: TableProps<T>) => {
    ...
    const renderItemPropContents = (i: ItemSchema<T>, item: T) => {
        return <>{typeof i.value === 'function' ? i.value(item) : i?.extractor ? i.extractor?.(resolveValue(item, `${i.property as string}`))?.key : booleanParser(resolveValue(item, `${i.property as string}`))} {i.units ? ' ' + i.units(item) : ''}</>
    }
        ...
        <Draggable key={`draggable-row-${item.id || item.title || item.dateCreated || JSON.stringify(item)}-${propKey}`} draggableId={`${item.id}`} index={index} isDragDisabled={!props.onDragEnd}>
            {(provided: any, snapshot) => (
                <React.Fragment key={`row-data-${index}-${item.id}-${propKey}`}>
                    ...
                    {Array.isArray((item as any).children) && !!(item as any).children?.length && Array.isArray(props.nestedSchema) && (<tr className="bg-mars-dark"><td className="p-0" colSpan={props.schema?.length}>

...
var TableLoader = function (props) {
    var propKey = props.hasOwnProperty("key") ? props.key || '' : '';
    ...
    var renderRowContents = function (item, snapshot) {
        ...
        if ((props === null || props === void 0 ? void 0 : props.onClick) && props.clickType === 'link') {
            rows.push(props.schema.map(function (i) { return React.createElement("td", { key: "row-prop-data-" + propKey + "-" + String(i.key || i.property) + "-link" },
            ...
            rows.push(props.schema.map(function (i) { return React.createElement("td", { key: "row-prop-data-" + propKey + "-" + String(i.key || i.property) + "-click" },
            ...
            rows.push(props.schema.map(function (i) { return React.createElement(TableCell, { cellClassName: props.cellClassName, snapshot: snapshot, id: String(i.key || i.property), key: "row-prop-data-" + propKey + "-" + String(i.key || i.property) }, renderItemProp(i, item)); }));
        }
        if (props.onUpdate) {
    ...
    var renderRow = function (item, index) { return (React.createElement(react_beautiful_dnd_1.Draggable, { key: item.id + "-" + propKey, draggableId: "" + item.id, index: index, isDragDisabled: !props.onDragEnd }, function (provided, snapshot) { return (React.createElement(React.Fragment, null,
        React.createElement("tr", (0, tslib_1.__assign)({ className: "bg-gradient-dark text-white " + (props.rowClassName || ''), key: "row-data-" + index + "-" + item.id + "-" + propKey, ref: provided.innerRef }, provided.draggableProps, provided.dragHandleProps, { style: provided.draggableProps.style }), renderRowContents(item, snapshot)),
        provided.placeholder)); })); };
    var renderHeader = function () { return (React.createElement("tr", { className: "bg-dark text-white" },
        props.schema.map(function (i) { return React.createElement("th", { style: i.labelStyle || {}, className: "" + (i.labelClassName || ''), key: "row-header-" + (i.property || i.label) + "-" + propKey }, i.label); }),

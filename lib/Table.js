...
var TableCell = function (_a) {
    var snapshot = _a.snapshot, children = _a.children, Wrapper = _a.Wrapper, row = _a.row, id = _a.id, props = (0, tslib_1.__rest)(_a, ["snapshot", "children", "Wrapper", "row", "id"]);
    ...
    return React.createElement("td", { ref: ref, className: "bg-dark", style: (snapshot === null || snapshot === void 0 ? void 0 : snapshot.isDragging) ? dimentionSnapshot || {} : {} }, children);
};
var TableLoader = function (props) {
    ...
    var renderRowContents = function (item, snapshot) {
        ...
        if ((props === null || props === void 0 ? void 0 : props.onClick) && props.clickType === 'link') {
            rows.push(props.schema.map(function (i) { return React.createElement("td", { key: "row-prop-data-" + String(i.key || i.property) + "-link" },
            ...
            rows.push(props.schema.map(function (i) { return React.createElement("td", { key: "row-prop-data-" + String(i.key || i.property) + "-click" },
            ...
            rows.push(props.schema.map(function (i) { return React.createElement(TableCell, { cellClassName: props.cellClassName, snapshot: snapshot, id: String(i.key || i.property), key: "row-prop-data-" + String(i.key || i.property) }, renderItemProp(i, item)); }));
        }
        if (props.onUpdate) {
            rows.push(React.createElement(TableCell, { snapshot: snapshot, key: "row-prop-data-update-" + JSON.stringify(item) },
    ...
    var renderRow = function (item, index) { return (React.createElement(react_beautiful_dnd_1.Draggable, { key: item.id, draggableId: "" + item.id, index: index, isDragDisabled: !props.onDragEnd }, function (provided, snapshot) { return (React.createElement(React.Fragment, null,
        React.createElement("tr", (0, tslib_1.__assign)({ className: "bg-gradient-dark text-white " + (props.rowClassName || ''), key: "row-data-" + index + "-" + item.id, ref: provided.innerRef }, provided.draggableProps, provided.dragHandleProps, { style: provided.draggableProps.style }), renderRowContents(item, snapshot)),
        provided.placeholder)); })); };
    var renderHeader = function () { return (React.createElement("tr", { className: "bg-dark text-white" },
        props.schema.map(function (i) { return React.createElement("th", { key: "row-header-" + i.label }, i.label); }),
    ...
    return (React.createElement(react_beautiful_dnd_1.DragDropContext, { onDragEnd: handleDragEnd },
        React.createElement(react_beautiful_dnd_1.Droppable, { droppableId: "droppable", isDropDisabled: !props.onDragEnd }, function (provided, snapshot) { return (React.createElement("table", { ref: provided.innerRef, className: "table table-hover table-dark" },
